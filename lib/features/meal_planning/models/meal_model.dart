...
/// Summary of daily nutrition generated by AI
...
/// Response containing daily summary and list of meals
...
class MealModel {
  ...
  /// Create from JSON (Daily Menu AI response)
  factory MealModel.fromDailyMenuJson(
    Map<String, dynamic> json, {
    Uint8List? imageBytes,
  }) {
    final typeStr = json['type'] as String? ?? 'lunch';
    final name = json['dish_name'] as String? ?? 'Untitled Meal';
    final searchTerm = _extractFoodKeyword(name);
    final placeholderImage = _generateFoodImageUrl(searchTerm);

    final nutrition = json['nutrition'] as Map<String, dynamic>? ?? {};

    // For DailyMenu, instructions is 'recipe' (string)
    final recipe = json['recipe'] as String? ?? '';
    final instructionsList = recipe.split(RegExp(r'\n+')).where((s) => s.trim().isNotEmpty).toList();

    return MealModel(
      id: DateTime.now().millisecondsSinceEpoch.toString() + '_' + typeStr,
      name: name,
      description: name,
      type: MealType.fromString(typeStr),
      ...
      imageUrl: placeholderImage,
      imagePrompt: json['image_description'] as String?,
      imageBytes: imageBytes,
      date: DateTime.now(),
      createdAt: DateTime.now(),
    );
